...
#    ACTIVATIONS = ['softmax', 'elu', 'selu', 'softplus', 'softsign', 'relu', 'tanh', 'sigmoid', 'hard_sigmoid', 'exponential', 'linear']
    ...
    for ACTIVATION in ACTIVATIONS:
        ...
        MODEL_FNAME = RELATIVE_PATH + 'my_first_mlp.h5'
        ...
        model.add(Dense(units=8, actvation='softmax'))
        print("here3")
        ...
        print(model.summary())
        plot_model(model, to_file=(RELATIVE_PATH + 'modelMLP.png'), show_shapes=True, show_layer_names=True)
        ...
        plt.savefig(RELATIVE_PATH  + 'accuracy.jpg')
        plt.close()
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'validation'], loc='upper left')
        plt.savefig(RELATIVE_PATH + 'loss.jpg')
